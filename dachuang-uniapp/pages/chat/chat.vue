...
const artifactId = ref(route.query.artifactId || '')
const artifactName = ref(route.query.artifactName || '')
...
const handleSend = async () => {
  ...
  try {
    ...
    const result = await pollTaskStatus(
      dialogTaskId,
      async (id) => {
        const statusRes = await dialogApi.getResult(id)
        return statusRes.data.status
      },
      async (id) => {
        return await dialogApi.getResult(id)
      }
    )
