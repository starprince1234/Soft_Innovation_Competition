      ...
      <view class="header-content">
        <view class="user-info" @click="goToProfile">
          ...
          <view class="user-text">
            ...
            <text class="role-name">{{ getRoleName(userInfo?.role) }}</text>
          </view>
        </view>
        <view class="header-actions">
          <view class="action-btn" @click="goToFeedback">
            <image class="action-icon" :src="feedbackIcon" mode="aspectFit"></image>
          </view>
        </view>
      </view>
    </view>
      ...
      <view class="quick-actions">
        <view class="action-card" @click="goToArtifacts">
          <view class="action-icon-wrapper" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
            <image class="action-icon" :src="artifactsIcon" mode="aspectFit"></image>
          </view>
        ...
        <view class="action-card" @click="goToChat">
          <view class="action-icon-wrapper" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
            <image class="action-icon" :src="chatIcon" mode="aspectFit"></image>
          </view>
        ...
        <view class="action-card" @click="goToHistory">
          <view class="action-icon-wrapper" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);">
            <image class="action-icon" :src="historyIcon" mode="aspectFit"></image>
          </view>
        ...
        <view class="action-card" @click="goToAbout">
          <view class="action-icon-wrapper" style="background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%);">
            <image class="action-icon" :src="aboutIcon" mode="aspectFit"></image>
          </view>
...
const banner1Image = resolveAssetPath('/static/images/banner1.png')
const banner2Image = resolveAssetPath('/static/images/banner2.png')
...
const handleScan = async () => {
  try {
    ...
    const result = await pollTaskStatus(
      taskId,
      async (id) => {
        const statusRes = await detectApi.getTaskStatus(id)
...
const saveToHistory = (detectResult) => {
  const firstMatch = detectResult?.detectedArtifacts?.[0]
  const historyItem = {
    id: firstMatch?.artifactId || detectResult?.taskId,
    image: detectResult?.imageUrl || emptyResultImage,
    name: firstMatch?.label || '识别结果',
    confidence: firstMatch?.confidence || 0
  }
