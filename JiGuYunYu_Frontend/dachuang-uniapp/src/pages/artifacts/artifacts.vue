  ...
  <view class="artifacts-page">
    <view class="page-header">
      ...
      <view class="filter-btn" @click="showFilter = true">
        <image class="filter-icon" :src="filterIcon" mode="aspectFit"></image>
...
const emptyArtifactsImage = resolveAssetPath('/static/images/empty-artifacts.png')
...
const filterIcon = resolveAssetPath('/static/icons/filter.svg')
...
const normalizePageData = (payload) => {
  ...
  return {
    ...
    currentPage: Number.isFinite(Number(pageData.currentPage))
      ? Number(pageData.currentPage)
      : pagination.value.page,
    pageSize: Number(pageData.pageSize) || pagination.value.pageSize
  }
}
...
const normalizeArtifact = (item) => ({
  ...
  imageUrl: normalizeOssUrl(item.thumbnailUrl || item.imageUrl, emptyResultImage),
...
const loadArtifacts = async (isRefresh = false) => {
  ...
  try {
    ...
    const normalizedTags = materialTagMap[filter.value.material] || filter.value.material || undefined
    const params = {
      page: pagination.value.page,
      size: pagination.value.pageSize,
      keyword: normalizedKeyword || undefined,
      name: normalizedKeyword || undefined,
      era: filter.value.period || undefined,
      tags: normalizedTags
    }

    const res = await artifactsApi.getList(params)
    const pageData = normalizePageData(res)
    const mappedList = pageData.list.map(normalizeArtifact)
    ...
    pagination.value.page = pageData.currentPage
    pagination.value.pageSize = pageData.pageSize
    hasMore.value = pageData.totalPages > 0 && (pageData.currentPage + 1) < pageData.totalPages
