  ...
  <view class="login-page">
    <view class="login-container">
      ...
      <view class="login-tabs">
        ...
        <view class="tab-item" :class="{ active: loginType === 'email' }" @click="switchLoginType('email')">
          <text class="tab-text">QQ邮箱登录</text>
        </view>
      </view>

      <view class="login-form">
        <block v-if="loginType === 'wechat'">
        ...
        <block v-else>
          ...
          <button class="email-login-btn" :class="{ disabled: !canEmailSubmit }" @click="handleEmailLogin">
            {{ emailLoading ? '登录中...' : '登录' }}
          </button>
        </block>
    ...
    <Loading :visible="loading || emailLoading" text="登录中..." />
  </view>
</template>
...
import { validateEmail } from '@/common/utils/index.js'
...
const canEmailSubmit = computed(() => {
  return validateEmail(formData.value.email) && formData.value.code.length === 6
})
...
const handleEmailLogin = async () => {
  ...
  try {
    const authRes = await authApi.emailLogin({
      email: formData.value.email,
